...
export const LearnPage = () => {
    ...
    const generate = useCallback(async () => {
        ...
        const preparedWords = _.slice(_.shuffle(words.filter(w => w.progress < 1)), 0, config.maxWords)
    ...
    const compare = useCallback(async (translation?: string) => {
        const words = await db.words.toArray()
            ...
            <Card>
                ...
                <CardActions>
                    <Grid container justifyContent='center'>
                        {translations.map(t => <Grid item key={t}><Button color="success" onClick={() => compare(t)}>{t}</Button></Grid>)}
                    </Grid>
                </CardActions>
                <CardActions>
                    <Grid container>
                        <Grid item>
                            <Button color="error" onClick={() => compare()}>I don't know</Button>
                        </Grid>
                    </Grid>
                </CardActions>
